		...
		foreach (spin_level, pattern; piece) {
			foreach (piece_seg; pattern) {
				...
				foreach (frame_seg; acc.frame) {
					...
					auto val = EvalFunc (acc.frame, moved);
					if (val == -float.infinity) continue;
					auto used_mask_cpy = acc.used_mask.dup;
					used_mask_cpy[piece_idx] = true;
					auto merged_frame = merge(acc.frame, moved);
					procedures ~=
...
Op[] greedy_search (alias EvalFunc)(in Piece[] pieces, in Shape frame) {
	...
	auto sorted = eval_all (piece_set, frame).sort;
...
unittest {
	...
	auto ops = beam_search!angle_and_points([p1, p2, p3], frame, 10);
}

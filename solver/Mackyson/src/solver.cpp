...
bool timeCheck(time_point start) {
	...
	return (now < start + std::chrono::minutes(15));
}
		...
		while (parEval[pieceNumber / 16].empty()) {			
			...
			for (int cnt = 0; cnt < pieceNumber / 16; ++cnt) {
				...
				for (int i = 0; i < width; ++i) {
					...
					climbForBeam(parEval[cnt].front(), &tmpEvalList, 30); //一回当たり、30個次の状態の候補を生成
					parEval[cnt].pop();
				}


				std::sort(tmpEvalList.rbegin(), tmpEvalList.rend());

				for (int i = 0; i < 30 && !tmpEvalList.empty(); ++i) {

...
public:
	...
	Frame(std::vector<Position>positionList) {
		//vertexPositionList.reserve(positionList.size() + 1); vectorのとき使用
		for (int i = 0; i < positionList.size(); ++i)
					...
					for (int i = 0;i < p.vertexPositionList.size();++i) {
						...
						if (p.vertexPositionList.at(i) == onlinePositionList.at(0)) {
							...
							for (int cnt = 0;cnt < p.vertexPositionList.size()-1;++cnt) {//挿入する頂点のリストをピースとして作成
								...
								i += i >= p.vertexPositionList.size()-1 ?  -(p.vertexPositionList.size()-1) : 1;
							}

							assert(p.vertexPositionList.size() - 1 == tmpPiece.vertexPositionList.size());
							
							std::vector<bool>willDeletedList;//死ぬ候補を作成
							...
							for (int cnt = 1;cnt < tmpPiece.vertexPositionList.size();++cnt)//死ぬ候補に該当する中で一個前の番号も死ぬ候補に該当する場合削除
